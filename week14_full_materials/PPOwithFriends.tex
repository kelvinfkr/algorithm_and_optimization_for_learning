...
要使$\Delta\theta = \alpha F^{-1} g = \sqrt{2\delta / (g^T F^{-1} g)} F^{-1} g$，需要：

$\frac{1}{\beta^*} = \sqrt{\frac{2\delta}{g^T F^{-1} g}}$

$\beta^* = \sqrt{\frac{g^T F^{-1} g}{2\delta}} = \frac{g^T F^{-1} g}{2\delta} \cdot \frac{1}{\sqrt{g^T F^{-1} g / (2\delta)}} = \frac{g^T F^{-1} g}{2\delta \alpha}$

当$\alpha = \sqrt{2\delta/(g^T F^{-1} g)}$时，$\beta^* = \frac{g^T F^{-1} g}{2\delta}$。
\end{proof}
...
\addplot[blue, thick, domain=0.8:1.2] {x};
\addplot[blue, thick, domain=1.2:2.5] {1.2};
\addplot[blue, dashed, domain=1.2:2.5] {x};

% A < 0 case  
\addplot[red, thick, domain=0:0.8] {-0.8};
\addplot[red, thick, domain=0.8:1.2] {-x};
\addplot[red, thick, domain=1.2:2.5] {-x};
\addplot[red, dashed, domain=0:0.8] {-x};

\legend{$A>0$实际, $A<0$实际}
\end{axis}
\end{tikzpicture}
...
\begin{proof}
\textbf{Step 1}：KL散度的定义

$D_{KL}(\pi_{\text{old}} \| \pi_\theta) = \mathbb{E}_{a \sim \pi_{\text{old}}}\left[\log \frac{\pi_{\text{old}}(a)}{\pi_\theta(a)}\right] = \mathbb{E}_{a \sim \pi_{\text{old}}}\left[-\log r\right]$

\textbf{Step 2}：约束$r \in [1-\epsilon, 1+\epsilon]$

$-\log r \in [-\log(1+\epsilon), -\log(1-\epsilon)]$

\textbf{Step 3}：Taylor展开
...
$D_{KL} = \mathbb{E}[-\log r]$

最坏情况：所有$r = 1-\epsilon$，则$D_{KL} = -\log(1-\epsilon) \approx \epsilon + \frac{\epsilon^2}{2}$

更精确的二阶展开：$-\log(1-\epsilon) = \epsilon + \frac{\epsilon^2}{2} + \frac{\epsilon^3}{3} + ... \leq \frac{\epsilon}{1-\epsilon}$

因此$D_{KL} \leq \frac{\epsilon}{1-\epsilon}$。

对于小$\epsilon$，$D_{KL} \lesssim \epsilon$。
\end{proof}
...
$\epsilon$ & $r$的范围 & KL上界 \\
\midrule
0.1 & $[0.9, 1.1]$ & $\approx 0.11$ \\
0.2 & $[0.8, 1.2]$ & $\approx 0.25$ \\
0.3 & $[0.7, 1.3]$ & $\approx 0.43$ \\
\bottomrule
\end{tabular}
\end{center}

\textbf{常用$\epsilon = 0.2$}对应KL约束$\approx 0.25$。

对比TRPO常用$\delta = 0.01$，PPO的"隐式约束"宽松得多！

\textbf{为什么还能工作？}因为clip不是硬约束，而是"软边界"——超出范围时梯度为零，但不禁止。
\end{calculation}

\begin{surprising}[$\epsilon = 0.2$为什么Work？]
从理论上看，$\epsilon = 0.2$允许KL高达0.25，远超TRPO的0.01。
...
\[
\nabla_\theta L^{\text{CLIP}} = \mathbb{E}\left[\nabla_\theta \log \pi_\theta(a|s) \cdot A \cdot \mathbf{1}[r \in \text{active region}]\right]
\]
其中active region是clip不起作用的区域：
\begin{itemize}
...
0.5 & -0.37 & 0.35 & -0.98 & 1.00 & -2.00 & 2.2步 \\
0.8 & -0.35 & 0.07 & -0.87 & 0.46 & -2.00 & 3.6步 \\
0.95 & -0.45 & -0.17 & -0.88 & 0.19 & -2.00 & 6.9步 \\
1 & -0.67 & -0.33 & -0.94 & 0.10 & -2.00 & 10步 \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{观察}：$\lambda$越大，$\hat{A}_0$越受远处（$r_3, r_4$）影响，绝对值越大。
\end{calculation}
...
设$V$有误差$\epsilon_V = V - V^{\pi}$，则：
\[
\text{Bias}[\hat{A}^{\text{GAE}}] = O\left(\frac{\gamma(1-\lambda)}{1-\gamma\lambda} \epsilon_V\right)
\]
\[
...
\begin{proof}
\textbf{Step 1}：DPO最优的充分条件

DPO损失为零当且仅当对所有$(y_w, y_l)$：
\[
\sigma\left(\beta \log \frac{\pi_\theta(y_w)}{\pi_{\text{ref}}(y_w)} - \beta \log \frac{\pi_\theta(y_l)}{\pi_{\text{ref}}(y_l)}\right) = 1
\]

这要求$\beta \log \frac{\pi_\theta(y_w)}{\pi_{\text{ref}}(y_w)} - \beta \log \frac{\pi_\theta(y_l)}{\pi_{\text{ref}}(y_l)} \to +\infty$
...
\midrule
0 & 0 & 1 & $\epsilon_V = 0.1$ & $\sigma_r^2 = 1$ \\
0.5 & 0.495 & 2 & $0.05$ & $1.3$ \\
0.9 & 0.891 & 9 & $0.011$ & $5.7$ \\
0.95 & 0.9405 & 17 & $0.006$ & $10$ \\
0.99 & 0.9801 & 50 & $0.002$ & $25$ \\
1 & 0.99 & 100 & $0$ & $50$ \\
\bottomrule
\end{tabular}
\end{center}
...
$\lambda = 0.95$：MSE $\approx 0.006^2 + 10 = 10.00004$

$\lambda = 0.9$：MSE $\approx 0.011^2 + 5.7 = 5.70012$

\textbf{$\lambda = 0.9$的MSE最小}！但差距不大，所以$[0.9, 0.99]$都可以。
\end{calculation}
